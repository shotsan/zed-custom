    ...
    pub fn render_footer(&self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let branch_picker_button = panel_button(branch)
            ...
            .on_click(cx.listener(|_, _, window, cx| {
                window.dispatch_action(zed_custom_actions::git::Branch.boxed_clone(), cx);
            }))
        ...
        let branch_picker = PopoverMenu::new("popover-button")
            ...
            .trigger_with_tooltip(
                ...
                Tooltip::for_action_title("Switch Branch", &zed_custom_actions::git::Branch),
            )
...
impl Render for CommitModal {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_action(
                cx.listener(|this, _: &zed_custom_actions::git::Branch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                }),
            )
            .on_action(
                cx.listener(|this, _: &zed_custom_actions::git::CheckoutBranch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                }),
            )
            .on_action(
                cx.listener(|this, _: &zed_custom_actions::git::Switch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                }),
            )
            .elevation_3(cx)
