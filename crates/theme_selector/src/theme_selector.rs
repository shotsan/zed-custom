...
use zed_custom_actions::{ExtensionCategoryFilter, Extensions};
...
pub fn init(cx: &mut App) {
    cx.on_action(|action: &zed_custom_actions::theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
            toggle_theme_selector(workspace, &action, window, cx);
        });
    });
    cx.on_action(|action: &zed_custom_actions::icon_theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
...
fn toggle_theme_selector(
    ...
    toggle: &zed_custom_actions::theme_selector::Toggle,
    window: &mut Window,
...
fn toggle_icon_theme_selector(
    workspace: &mut Workspace,
    toggle: &zed_custom_actions::icon_theme_selector::Toggle,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    let fs = workspace.app_state().fs.clone();
