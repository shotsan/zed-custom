...
async fn test_sharing_an_ssh_remote_project(
    ...
    worktree_a.update(cx_a, |worktree, _cx| {
        assert_eq!(
            ...
            vec![
                rel_path(".zed_custom"),
                rel_path(".zed_custom/settings.json"),
                rel_path("README.md"),
                rel_path("src"),
                rel_path("src/lib.rs"),
            ]
...
async fn test_ssh_remote_worktree_trust(cx_a: &mut TestAppContext, server_cx: &mut TestAppContext) {
    ...
    cx_a.read(|cx| {
        ...
        assert_eq!(
            language_settings(Some("Rust".into()), file, cx).language_servers,
            ["...".to_string()],
            "remote .zed_custom/settings.json must not sync before trust approval"
        )
    });
    ...
    cx_a.read(|cx| {
        ...
        assert_eq!(
            language_settings(Some("Rust".into()), file, cx).language_servers,
            ["override-rust-analyzer".to_string()],
            "remote .zed_custom/settings.json should sync after trust approval"
        )
    });
