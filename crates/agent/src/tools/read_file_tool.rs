...
mod test {
    ...
    async fn test_read_nonexistent_file(cx: &mut TestAppContext) {
        ...
        let memory_db = MemoryDatabase::new(cx.executor(), Connection::open_memory(None)).unwrap();
    ...
    async fn test_read_file_security(cx: &mut TestAppContext) {
        ...
        let model = Arc::new(FakeLanguageModel::default());
        let memory_db = MemoryDatabase::new(cx.executor(), Connection::open_memory(None)).unwrap();
        let memory_store = Arc::new(MemoryStore::new(Arc::new(memory_db), "/project_root".into()));
        let semantic_index = Arc::new(parking_lot::RwLock::new(SemanticIndex::new()));
        let thread = cx.new(|cx| {
            Thread::new(
                project.clone(),
    ...
    async fn test_read_file_with_multiple_worktree_settings(cx: &mut TestAppContext) {
        ...
        let model = Arc::new(FakeLanguageModel::default());
        let memory_db = MemoryDatabase::new(cx.executor(), Connection::open_memory(None)).unwrap();
        let memory_store = Arc::new(MemoryStore::new(Arc::new(memory_db), "/worktree1".into()));
        let semantic_index = Arc::new(parking_lot::RwLock::new(SemanticIndex::new()));
        let thread = cx.new(|cx| {
            Thread::new(
                project.clone(),
