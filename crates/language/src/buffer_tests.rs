...
fn test_select_language(cx: &mut App) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("zed_custom/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("zed_custom/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("zed_custom/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
