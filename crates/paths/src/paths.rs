...
/// On macOS, this is `~/Library/Application Support/zed-custom`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/zed-custom`.
/// On Windows, this is `%LOCALAPPDATA%\zed-custom`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/zed-custom`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/zed-custom`.
/// On Windows, this is `%APPDATA%\zed-custom`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the zed_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zed-custom_server").unwrap());
    *CACHED
}
...
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zed-custom_wsl_server").unwrap());
    *CACHED
}
...
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("zed-custom")
        }
    })
}
...
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join("Library/Application Support/zed-custom")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("zed-custom")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/zed-custom")
        } else {
            data_dir().join("logs")
        }
    })
...
pub fn log_file() -> &'static PathBuf {
    ...
    LOG_FILE.get_or_init(|| logs_dir().join("zed-custom.log"))
}

/// Returns the path to the `Zed.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("zed-custom.log.old"))
}
...
/// Returns the relative path to a `.zed_custom` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zed_custom/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zed_custom/tasks.json").unwrap());
    *CACHED
}
...
/// .zed_custom/debug.json
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zed_custom/debug.json").unwrap());
    *CACHED
}
