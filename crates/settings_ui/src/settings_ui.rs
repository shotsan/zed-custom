...
use zed_custom_actions::{OpenProjectSettings, OpenSettings, OpenSettingsAt};
        ...
        cx.open_window(
            WindowOptions {
                titlebar: Some(TitlebarOptions {
                    title: Some("zed-custom — Settings".into()),
...
fn render_settings_item_link(
    ...
    let clipboard_has_link = cx
        ...
        .map_or(false, |maybe_url| {
            json_path.is_some() && maybe_url.strip_prefix("zed-custom://settings/") == json_path
        });
            ...
            IconButton::new((id.into(), "copy-link-btn"), link_icon)
                ...
                .when_some(json_path, |this, path| {
                    this.on_click(cx.listener(move |_, _, _, cx| {
                        let link = format!("zed-custom://settings/{}", path);
    ...
    workspace::AppState::global(cx)
        ...
        .map_or(vec![], |state| {
            state
                ...
                .language_names()
                .into_iter()
                .filter(|name| name.as_ref() != "zed-custom Keybind Context")
                .map(Into::into)
                .collect()
        })
}
    ...
    fn open_current_settings_file(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        match &self.current_file {
            ...
            SettingsUiFile::Project((worktree_id, path)) => {
                ...
                // TODO: move zed_custom::open_local_file() APIs to this crate, and
...
mod project_settings_update_tests {
    ...
    async fn init_test(cx: &mut TestAppContext, initial_settings: Option<&str>) -> TestSetup {
        ...
        let rel_path: Arc<RelPath> = RelPath::unix(".zed_custom/settings.json")
            .expect("valid path")
            .into_arc();
        let project_path = ProjectPath {
            worktree_id,
    ...
    async fn test_handles_dropped_worktree(cx: &mut TestAppContext) {
        ...
        let file_content = setup
            .fs
            .load("/project/.zed_custom/settings.json".as_ref())
            .await
            .unwrap();
        assert_eq!(
            file_content, "{}",
    ...
    async fn test_reloads_conflicted_buffer(cx: &mut TestAppContext) {
        ...
        setup
            .fs
            .save(
                "/project/.zed_custom/settings.json".as_ref(),
                &r#"{ "tab_size": 99 }"#.into(),
                Default::default(),
            )
            .await
