...
use zed_custom_actions::ExtensionCategoryFilter;
...
pub fn init(cx: &mut App) {
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        workspace
            .register_action(
                move |workspace, action: &zed_custom_actions::Extensions, window, cx| {
    ...
    fn on_extension_installed(
            ...
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        zed_custom_actions::theme_selector::Toggle {
        ...
        if !icon_themes.is_empty() {
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        zed_custom_actions::icon_theme_selector::Toggle {
                            ...
                            .when(!is_compatible, |upgrade_button| {
                                upgrade_button.disabled(true).tooltip({
                                    ...
                                    move |_, cx| {
                                        Tooltip::simple(
                                            format!(
                                                "v{version} is not compatible with this version of zed-custom.",
                                            ),
                                             cx,
                                        )
                                    }
        ...
        let view_registry = Button::new("view_registry", "View Registry")
            ...
            .on_click({
                ...
                move |_, window, cx| {
                    ...
                    window.dispatch_action(Box::new(zed_custom_actions::AcpRegistry), cx)
                }
            });
        ...
        for feature in &self.upsells {
            let banner = match feature {
                ...
                Feature::ExtensionTailwind => self.render_feature_upsell_banner(
                    "Tailwind CSS support is built-in to zed-custom!".into(),
                    "https://zed.dev/docs/languages/tailwindcss".into(),
                    false,
                    cx,
                ),
                Feature::ExtensionTy => self.render_feature_upsell_banner(
                    "Ty (Python language server) support is built-in to zed-custom!".into(),
                    "https://zed.dev/docs/languages/python".into(),
                    false,
                    cx,
                ),
                Feature::Git => self.render_feature_upsell_banner(
                    "zed-custom comes with basic Git support—more features are coming in the future."
                        .into(),
