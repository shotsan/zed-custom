...
// If the zed_custom binary doesn't use anything in this crate, it will be optimized away
...
/// Opens a zed-custom:// URL within the application.
#[derive(Clone, PartialEq, Deserialize, JsonSchema, Action)]
#[action(namespace = zed)]
#[serde(deny_unknown_fields)]
...
pub mod assistant {
    ...
    /// Opens the skill library for managing agent skills and prompts.
    #[derive(PartialEq, Clone, Default, Debug, Deserialize, JsonSchema, Action)]
    #[action(namespace = agent, deprecated_aliases = ["agent::OpenRulesLibrary", "assistant::OpenRulesLibrary", "assistant::DeployPromptLibrary"])]
    #[serde(deny_unknown_fields)]
    pub struct OpenSkillLibrary {
        #[serde(skip)]
