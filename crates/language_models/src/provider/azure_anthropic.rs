...
use ui::{ButtonLink, List, ListBulletItem, prelude::*};
use ui_input::InputField;
use util::ResultExt;
use fs::Fs;
use settings::update_settings_file;

use super::anthropic::AnthropicSettings;

const PROVIDER_ID: LanguageModelProviderId = language_model::AZURE_ANTHROPIC_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = language_model::AZURE_ANTHROPIC_PROVIDER_NAME;

pub struct AzureAnthropicLanguageModelProvider {
    http_client: Arc<dyn HttpClient>,
    state: Entity<State>,
}

const AZURE_ANTHROPIC_API_KEY_ENV_VAR_NAME: &str = "AZURE_ANTHROPIC_API_KEY";
static API_KEY_ENV_VAR: LazyLock<EnvVar> = env_var!(AZURE_ANTHROPIC_API_KEY_ENV_VAR_NAME);
...
impl State {
    ...
    fn set_api_key(&mut self, api_key: Option<String>, cx: &mut Context<Self>) -> Task<Result<()>> {
        let api_url = AzureAnthropicLanguageModelProvider::api_url(cx);
        self.api_key_state
...
impl AzureAnthropicLanguageModelProvider {
    ...
    fn settings(cx: &App) -> &AnthropicSettings {
        &crate::AllLanguageModelSettings::get_global(cx).azure_anthropic
    }
...
impl LanguageModelProviderState for AzureAnthropicLanguageModelProvider {
...
impl LanguageModelProvider for AzureAnthropicLanguageModelProvider {
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        for model in &AzureAnthropicLanguageModelProvider::settings(cx).available_models {
            models.insert(
                model.name.clone(),
...
pub struct AzureAnthropicModel {
    id: LanguageModelId,
...
impl AzureAnthropicModel {
    ...
    > {
        ...
        let (api_key, api_url) = self.state.read_with(cx, |state, cx| {
            let api_url = AzureAnthropicLanguageModelProvider::api_url(cx);
            let api_key = AzureAnthropicLanguageModelProvider::settings(cx)
                .api_key
                .clone()
...
impl LanguageModel for AzureAnthropicModel {
    ...
    fn api_key(&self, cx: &App) -> Option<String> {
        self.state.read_with(cx, |state, cx| {
            let api_url = AzureAnthropicLanguageModelProvider::api_url(cx);
            AzureAnthropicLanguageModelProvider::settings(cx)
                .api_key
                .clone()
    ...
    > {
        let model_name = {
            let api_url = self.state.read_with(cx, |_state, cx| {
                AzureAnthropicLanguageModelProvider::api_url(cx)
            });
...
impl ConfigurationView {
    ...
    fn new(
        ...
        let settings = AzureAnthropicLanguageModelProvider::settings(cx);
        let api_url = settings.api_url.clone();
        let model = settings.available_models.first().cloned();

        let api_key_editor = cx.new(|cx| InputField::new(window, cx, Self::PLACEHOLDER_TEXT).label("API Key"));
        let api_url_editor = cx.new(|cx| {
            let editor = InputField::new(window, cx, "https://<resource>.services.ai.azure.com/anthropic").label("Endpoint URL");
            editor.set_text(&api_url, window, cx);
            editor
        });
        let deployment_name_editor = cx.new(|cx| {
            let editor = InputField::new(window, cx, "my-deployment").label("Deployment Name");
        ...
        let max_output_tokens_editor = cx.new(|cx| {
            let editor = InputField::new(window, cx, "32000").label("Max Output Tokens");
            if let Some(model) = &model {
    ...
    fn reset_settings(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        update_settings_file(fs, cx, |settings, _| {
            if let Some(azure) = settings
                .language_models
                .as_mut()
                .and_then(|models| models.azure_anthropic.as_mut())
            {
                azure.api_url = None;
                azure.api_key = None;
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(
                    v_flex()
                        ...
                        .child(Label::new(format!("To use {}, you need to configure your endpoint and deployment. Follow these steps:", match &self.target_agent {
                            ConfigurationViewTargetAgent::ZedAgent => "Zed's agent with Azure Anthropic".into(),
                ...
                .child(
                    Label::new(
                        format!("You can also set the {AZURE_ANTHROPIC_API_KEY_ENV_VAR_NAME} environment variable and restart zed-custom."),
                    )
