...
pub enum CommandScope {
    /// Project-specific command from .zed_custom/commands/
    Project,
...
pub fn project_commands_dir(worktree_root: &Path) -> PathBuf {
    worktree_root.join(".zed_custom").join("commands")
}
...
/// Returns the path to the project skills directory for a given worktree root.
pub fn project_skills_dir(worktree_root: &Path) -> PathBuf {
    worktree_root.join(".zed_custom").join("skills")
}

/// Events emitted by UserSlashCommandRegistry
#[derive(Debug, Clone)]
#[allow(dead_code)] // Infrastructure for future caching implementation
pub enum UserSlashCommandRegistryEvent {
...
impl UserSlashCommandRegistry {
    ...
    fn start_watching(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let mut dirs_to_watch = vec![user_commands_dir(), user_skills_dir()];
            for root in &worktree_roots {
                dirs_to_watch.push(project_commands_dir(root));
                dirs_to_watch.push(project_skills_dir(root));
            }
...
pub async fn load_all_commands_async(
    ...
    // Project paths first (higher precedence)
    for root in worktree_roots {
        paths_to_load.push((project_commands_dir(root), CommandScope::Project));
        paths_to_load.push((project_skills_dir(root), CommandScope::Project));
    }

    // User paths
    paths_to_load.push((user_commands_dir(), CommandScope::User));
    paths_to_load.push((user_skills_dir(), CommandScope::User));

    for (commands_path, scope) in paths_to_load {
        let folder_result = load_commands_from_path_async(fs, &commands_path, scope).await;
        result.errors.extend(folder_result.errors);
        for cmd in folder_result.commands {
...
mod tests {
    ...
    async fn test_registry_updates_worktree_roots(cx: &mut TestAppContext) {
        ...
        registry.update(cx, |registry: &mut UserSlashCommandRegistry, cx| {
    ...
    async fn test_load_commands_claude_symlink_pattern(cx: &mut TestAppContext) {
        // Simulates the common pattern of symlinking ~/.claude/commands/ to zed_custom's commands dir
        let fs = FakeFs::new(cx.executor());
