...
pub(crate) fn create_dialog_window(receiver: Receiver<Result<()>>) -> Result<HWND> {
    unsafe {
        let class_name = windows::core::w!("zed-custom-Auto-Updater-Dialog-Class");
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        ...
        WM_PAINT => unsafe {
            ...
            let string = HSTRING::from("Updating zed-custom...");
        ...
        WM_TERMINATE => {
            with_dialog_data(hwnd, |data| {
                if let Ok(result) = data.borrow_mut().rx.recv()
                    && let Err(e) = result
                {
                    log::error!("Failed to update zed-custom: {:?}", e);
