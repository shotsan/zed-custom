...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("zed-custom.exe"), p("old\\zed-custom.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\zed-custom.exe"), p("old\\bin\\zed-custom.exe")),
        Job::move_file(p("bin\\zed_custom"), p("old\\bin\\zed_custom")),
        //
        ...
        // Copy new files
        Job::move_file(p("install\\zed-custom.exe"), p("zed-custom.exe")),
        Job::move_file(p("install\\bin\\zed-custom.exe"), p("bin\\zed-custom.exe")),
        Job::move_file(p("install\\bin\\zed_custom"), p("bin\\zed_custom")),
        //
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("zed-custom.exe")).spawn();
    }
