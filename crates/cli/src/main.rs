...
const URL_PREFIX: [&'static str; 5] = ["zed-custom://", "http://", "https://", "file://", "ssh://"];
...
#[command(
    ...
    before_help = "The zed-custom CLI binary.
This CLI is a separate binary that invokes zed-custom.

Examples:
    `zed-custom`
          Simply opens zed-custom
    `zed-custom --foreground`
          Runs in foreground (shows all logs)
    `zed-custom path-to-your-project`
          Open your project in zed-custom
    `zed-custom -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | zed-custom -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/zed-custom`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\zed-custom`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/zed_custom`."
    )]
    ...
    #[arg(short, long)]
    version: bool,
    /// Run zed_custom in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to zed-custom.app or the zed_custom binary
    #[arg(long)]
    zed_custom: Option<PathBuf>,
    /// Run zed_custom in dev-server mode
    #[arg(long)]
...
///
/// This method must return an absolute path, as many zed_custom
...
fn main() -> Result<()> {
    ...
    // `zed_custom --askpass` Makes zed_custom operate in nc/netcat mode for use with askpass
    ...
    let app = Detect::detect(args.zed_custom.as_deref()).context("Bundle detection")?;
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Zed CLI, only on zed-custom binary.",
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before zed-custom spawn")?;
    let url = format!("zed-custom-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `zed_custom`.
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after zed-custom spawn")?;
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/zed_custom is for Arch (and other non-libexec distros),
                // ./zed_custom is for the target directory in development builds.
                let possible_locations =
                    ["../libexec/zed-custom", "../lib/zed-custom/zed-custom", "./zed-custom"];
                possible_locations
                    .iter()
...
mod flatpak {
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=ZED_UPDATE_EXPLANATION=Please use flatpak to update zed_custom".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("zed_custom").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--zed_custom";
            }

            if !is_app_location_set {
                args.push("--zed_custom".into());
                args.push(flatpak_dir.join("libexec").join("zed-custom").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("dev.zed-custom.zed-custom-Custom"))
            && args.zed_custom.is_none()
        {
            args.zed_custom = Some("/app/libexec/zed-custom".into());
            unsafe { env::set_var("ZED_UPDATE_EXPLANATION", "Please use flatpak to update zed_custom") };
        }
        args
    }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.zed_custom.zed-custom") {
                return None;
            }
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../Zed.exe is the standard, lib/zed_custom is for MSYS2, ./zed_custom.exe is for the target
                // directory in development builds.
                let possible_locations = ["../zed-custom.exe", "../lib/zed-custom/zed-custom.exe", "./zed-custom.exe"];
                possible_locations
                    .iter()
                    .find_map(|p| dir.join(p).canonicalize().ok().filter(|path| path != &cli))
                    .context(format!(
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("zed-custom {} – {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open zed_custom-cli:... -a /Applications/Zed\ Preview.app
                        let urls_to_open =
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/zed-custom"),
