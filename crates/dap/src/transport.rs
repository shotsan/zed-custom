...
impl StdioTransport {
    // #[allow(dead_code, reason = "This is used in non test builds of zed-custom")]
    async fn start(
        binary: &DebugAdapterBinary,
        log_handlers: LogHandlers,
        cx: &mut AsyncApp,
        ...
        let Some(binary_command) = &binary.command else {
            bail!(
                "When using the `stdio` transport, the path to a debug adapter binary must be set by zed-custom."
            );
        };
