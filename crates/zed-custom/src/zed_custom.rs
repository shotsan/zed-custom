...
actions!(
    dev,
    [
        /// Stores last 30s of audio from zed_custom staff using the experimental rodio
...
pub fn init(cx: &mut App) {
    ...
    .on_action(|_: &zed_custom_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&zed_custom_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &zed_custom_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|_: &zed_custom_actions::About, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
...
fn unstable_version_notification(cx: &mut App) {
    ...
    show_app_notification(NotificationId::unique::<WindowsNightly>(), cx, |cx| {
        cx.new(|cx| {
            MessageNotification::new("You're using an unstable version of zed-custom (Nightly)", cx)
                .primary_message("Download Stable")
                .primary_icon_color(Color::Accent)
                .primary_icon(IconName::Download)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(
                        zed_custom_actions::OpenBrowser {
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            ...
            (
                ...
                "https://zed.dev/docs/linux#zed_custom-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
...
fn initialize_panels(
    ...
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let slack_panel = zed_custom_slack::slack_panel::SlackPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(slack_panel, workspace_handle.clone(), cx.clone()),
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|workspace, action: &zed_custom_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &zed_custom_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zed_custom_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterZedScheme, window, cx| {
                ...
                workspace.update_in(cx, |workspace, _, cx| {
                    ...
                    workspace.show_toast(
                        Toast::new(
                            ...
                            format!(
                                "zed-custom:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering zed-custom:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &zed_custom_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &zed_custom_actions::agent::ToggleAgentPane,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
                ...
                show_app_notification(id, cx, move |cx| {
                    cx.new(|cx| {
                        MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                            ...
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    zed_custom_actions::OpenSettingsFile.boxed_clone(),
                                    cx,
                                );
                                cx.emit(DismissEvent);
                            })
            ...
            if !showed_parse_error {
                show_app_notification(id, cx, move |cx| {
                    cx.new(|cx| {
                        ...
                        .primary_icon(IconName::Settings)
                        .primary_on_click(|window, cx| {
                            window.dispatch_action(zed_custom_actions::OpenSettingsFile.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                    })
                });
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(zed_custom_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &zed_custom_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .zed_custom/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.zed_custom/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .zed_custom to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".zed_custom".to_string()]);
            });
        });

        eprintln!("Added .zed_custom to file_scan_exclusions in settings");

        // 4. Run tasks to apply settings
        cx.background_executor.run_until_parked();

        // 5. Critical: Verify .zed_custom is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap());

        let has_zed_entry =
            cx.update(|cx| worktree.read(cx).entry_for_path(rel_path(".zed_custom")).is_some());

        eprintln!(
            "Is .zed_custom directory visible in worktree after exclusion: {}",
            has_zed_entry
        );

        // This assertion verifies the test is set up correctly to show the bug
        // If .zed_custom is not excluded, the test will fail here
        assert!(
            !has_zed_entry,
            "Test precondition failed: .zed_custom directory should be excluded but was found in worktree"
        );
