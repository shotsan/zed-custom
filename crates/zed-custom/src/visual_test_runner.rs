...
//!   cargo run -p zed_custom --bin zed_visual_test_runner --features visual-tests
//!
//! Update baseline images (when UI intentionally changes):
//!   UPDATE_BASELINE=1 cargo run -p zed_custom --bin zed_visual_test_runner --features visual-tests
//!
...
fn main() {
    ...
    // This must be done before any code accesses zed_custom_env_vars::ZED_STATELESS
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the zed-custom logo".into()], cx)
    });
