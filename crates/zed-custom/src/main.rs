...
use crate::zed_custom::{OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "zed-custom failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "zed-custom failed to open a window: {e:?}. See https://zed.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.zed-custom.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    // `zed-custom --askpass` Makes zed-custom operate in nc/netcat mode for use with askpass
    ...
    // `zed-custom --crash-handler` Makes zed-custom operate in minidump crash handler mode
    if let Some(socket) = &args.crash_handler {
        crashes::crash_server(socket.as_path());
        return;
    }

    // `zed-custom --nc` Makes zed-custom operate in nc/netcat mode for use with MCP
    ...
    // `zed-custom --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    if args.system_specs {
        ...
        println!("zed-custom System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    log::info!(
        "========== starting zed-custom version {}, sha {} ==========",
        app_version,
        app_commit_sha
            .as_ref()
            .map(|sha| sha.short())
    ...
    let failed_single_instance_check = if *zed_custom_env_vars::ZED_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::zed_custom::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::zed_custom::windows_only_instance::handle_single_instance(open_listener.clone(), &args)
        }

        #[cfg(target_os = "macos")]
        {
            use zed_custom::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("zed_custom is already running");
        return;
    }
    ...
    app.run(move |cx| {
        ...
        acp_tools::init(cx);
        zed_custom::telemetry_log::init(cx);
        zed_custom::remote_debug::init(cx);
    ...
    if let Some(kind) = request.kind {
        match kind {
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("zed-custom://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // zed-custom://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // zed-custom://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(zed_custom_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(zed_custom_actions::OpenSettingsAt { path: setting_path }),
                            cx,
                        ),
                    })
                })
...
#[derive(Parser, Debug)]
#[command(name = "zed_custom", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `zed-custom://` scheme, or relative to <https://zed.dev>.
    ...
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed_custom`.
    ...
    /// Instructs zed_custom to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Zed from starting, so you can't run `zed_custom: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,
    ...
    /// Run zed_custom in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                ...
                || arg.starts_with("zed-custom-cli://")
                || arg.starts_with("ssh://")
