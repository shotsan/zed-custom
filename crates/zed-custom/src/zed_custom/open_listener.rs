...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("zed-custom-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("zed_custom-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("zed-custom://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("zed-custom://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("zed-custom://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if let Some(agent_path) = url.strip_prefix("zed-custom://agent") {
                this.parse_agent_url(agent_path)
            } else if let Some(session_id_str) = url.strip_prefix("zed-custom://agent/shared/") {
            ...
            } else if let Some(schema_path) = url.strip_prefix("zed-custom://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "zed-custom://settings" || url == "zed-custom://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("zed-custom://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("zed-custom://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("zed-custom://git/commit/") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("zed-custom-{}.sock", *RELEASE_CHANNEL_NAME));
...
mod tests {
    use super::*;
    use crate::zed_custom::{open_listener::open_local_workspace, tests::init_test};
    use cli::{
        CliResponse,
        ipc::{self},
    };
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zed-custom://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zed-custom://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["zed-custom://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zed-custom://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "zed-custom://git/clone/?repo=https://github.com/zed-industries/zed.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "zed-custom://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fzed-industries%2Fzed.git"
                            .into(),
                    ],
                    ..Default::default()
                },
