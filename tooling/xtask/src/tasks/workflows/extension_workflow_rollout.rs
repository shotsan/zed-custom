...
fn rollout_workflows_to_extension(fetch_repos_job: &NamedJob) -> NamedJob {
    ...
    fn sync_workflow_files(removed_files: &StepOutput) -> Step<Run> {
        named::bash(formatdoc! {r#"
            ...
            if [ "${{{{ matrix.repo }}}}" = "workflows" ]; then
                cp zed-custom/extensions/workflows/*.yml extension/.github/workflows/
            else
                cp zed-custom/extensions/workflows/shared/*.yml extension/.github/workflows/
            fi
        "#})
    }
    ...
    fn create_pull_request(token: &StepOutput, short_sha: &StepOutput) -> Step<Use> {
        let title = format!("Update CI workflows to `zed-custom@{}`", short_sha);
        ...
        named::uses("peter-evans", "create-pull-request", "v7")
            ...
            .add_with((
                ...
                indoc::indoc! {r#"
                    ...
                    based on the commit shotsan/zed-custom@${{ github.sha }}
                "#},
            ))
    ...
    let (authenticate, token) = generate_token(
        ...
        Some(
            RepositoryTarget::new("zed-custom-extensions", &["${{ matrix.repo }}"]).permissions([
...
fn create_rollout_tag(rollout_job: &NamedJob) -> NamedJob {
    ...
    fn configure_git() -> Step<Run> {
        named::bash(indoc! {r#"
            git config user.name "zed-custom-zippy[bot]"
            git config user.email "234243425+zed-custom-zippy[bot]@users.noreply.github.com"
        "#})
    }
