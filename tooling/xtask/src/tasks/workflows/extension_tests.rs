...
// This is used by various extensions repos in the zed-custom-extensions org to run automated tests.
...
pub fn cache_zed_extension_cli() -> (Step<Use>, StepOutput) {
    ...
    .id("cache-zed-custom-extension-cli")
    .with(
        Input::default()
            .add("path", "zed-custom-extension")
            .add("key", "zed-custom-extension-${{ env.ZED_EXTENSION_CLI_SHA }}"),
    );
...
pub fn download_zed_extension_cli(cache_hit: StepOutput) -> Step<Run> {
    ...
    indoc! {
        r#"
        wget --quiet "https://zed-custom-extension-cli.nyc3.digitaloceanspaces.com/$ZED_EXTENSION_CLI_SHA/x86_64-unknown-linux-gnu/zed-custom-extension"
        chmod +x zed-custom-extension
        "#,
    }
...
pub fn check() -> Step<Run> {
    named::bash(indoc! {
        ...
        ./zed-custom-extension --source-dir . --scratch-dir /tmp/ext-scratch --output-dir /tmp/ext-output
        "#
    })
}
