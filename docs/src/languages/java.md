...
Java language support in zed-custom is provided by:

- zed-custom Java: [zed-custom-extensions/java](https://github.com/zed-custom-extensions/java)
...
You can install by opening {#action zed-custom::Extensions}({#kb zed-custom::Extensions}) and searching for `java`.

## Quick start and configuration

For the majority of users, Java support should work out of the box.

- It is generally recommended to open projects with the zed-custom-project root at the Java project root folder (where you would commonly have your `pom.xml` or `build.gradle` file).
...
Debug support is enabled via our [Fork of Java Debug](https://github.com/zed-industries/java-debug), which the extension will automatically download and start for you. Please refer to the [Debugger Documentation](https://zed.dev/docs/debugger#getting-started) for general information about how debugging works in zed-custom.
...
This extension provides tasks for running your application and tests from within zed-custom via little play buttons next to tests/entry points. However, due to current limitations of zed-custom's extension interface, we can not provide scripts that will work across Maven and Gradle on both Windows and Unix-compatible systems, so out of the box the launch scripts only work on Mac and Linux.

There is a fairly straightforward fix that you can apply to make it work on Windows by supplying your own task scripts. Please see [this Issue](https://github.com/zed-custom-extensions/java/issues/94) for information on how to do that and read the [Tasks section in Zeds documentation](https://zed.dev/docs/tasks) for more information.
    ...
    "jdtls": {
      "settings": {
        ...
        "initialization_options": {
          "bundles": [],
          // use this if your zed-custom project root folder is not the same as the java project root:
...
[zed-custom Java Repo](https://github.com/zed-custom-extensions/java)
