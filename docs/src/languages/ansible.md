...
Support for Ansible in zed-custom is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/zed-custom-ansible).
...
To avoid mishandling non-Ansible YAML files, the Ansible Language is not associated with any file extensions by default. To change this behavior you can add a `"file_types"` section to zed-custom settings inside your project (`.zed-custom/settings.json`) or your zed-custom user settings (`~/.config/zed-custom/settings.json`) to match your folder/naming conventions. For example:
...
- Or configure the yaml language server settings to set this schema for all your inventory files, that match your inventory pattern, under your zed-custom settings ([ref](https://zed.dev/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your zed-custom settings file. For example:

```json [settings]
"lsp": {
  // Note, the zed-custom Ansible extension prefixes all settings with `ansible`
