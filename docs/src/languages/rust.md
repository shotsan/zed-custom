...
Rust support is available natively in zed-custom.
...
<!--
TBD: Polish Rust Docs. zed-custom is a good rust editor, good Rust docs make it look like we care about Rust (we do!)
...
You can configure which `rust-analyzer` binary zed-custom should use.

By default, zed-custom will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, zed-custom will fall back to installing its own stable `rust-analyzer` version and use that.

If you want to install pre-release `rust-analyzer` version instead you can instruct zed-custom to do so by setting `pre_release` to `true` in your `settings.json`:
...
If you want to disable zed-custom looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If you want rust-analyzer to provide diagnostics for a target other than your current platform (e.g. for windows when running on macOS) you can use the following zed-custom lsp settings:
...
zed-custom provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
<!--
TBD: Is it possible to specify RUSTFLAGS? https://github.com/zed-industries/zed-custom/issues/14334
-->
...
Rust-analyzer in zed-custom runs with the default parameters.
...
Which would mean that every time zed-custom saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as zed-custom already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for zed-custom settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
...
zed-custom supports debugging Rust binaries and tests out of the box with `CodeLLDB` and `GDB`. Run {#action debugger::Start} ({#kb debugger::Start}) to launch one of these preconfigured debug tasks.

For more control, you can add debug configurations to `.zed-custom/debug.json`. See the examples below.
...
When you use `cargo build` or `cargo test` as the build command, zed-custom can infer the path to the output binary.
