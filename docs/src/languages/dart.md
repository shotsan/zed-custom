...
Dart support is available through the [Dart extension](https://github.com/zed-custom-extensions/dart).
...
If you would like to use a specific dart binary or use dart via FVM you can specify the `dart` binary in your zed-custom settings.jsons file:
...
Dart by-default uses a very conservative maximum line length (80). If you would like the dart LSP to permit a longer line length when auto-formatting, add the following to your zed-custom settings.json:
