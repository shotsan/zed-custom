...
Ruby support is available through the [Ruby extension](https://github.com/zed-custom-extensions/ruby).
...
There are multiple language servers available for Ruby. zed-custom supports the two following:
...
In addition to these two language servers, zed-custom also supports:

- [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by zed-custom as a language server, but its functionality is complimentary to that of solargraph and ruby-lsp.
...
The [Ruby extension](https://github.com/zed-custom-extensions/ruby) offers both `solargraph` and `ruby-lsp` language server support.
...
Solargraph has formatting and diagnostics disabled by default. We can tell zed-custom to enable them by adding the following to your `settings.json`:
...
LSP `settings` and `initialization_options` can also be project-specific. For example to use [standardrb/standard](https://github.com/standardrb/standard) as a formatter and linter for a particular project, add this to a `.zed-custom/settings.json` inside your project repo:
...
Rubocop has unsafe autocorrection disabled by default. We can tell zed-custom to enable it by adding the following to your `settings.json`:
...
To run tests in your Ruby project, you can set up custom tasks in your local `.zed-custom/tasks.json` configuration file. These tasks can be defined to work with different test frameworks like Minitest, RSpec, quickdraw, and tldr. Below are some examples of how to set up these tasks to run your tests from within your editor.
...
The Ruby extension provides a debug adapter for debugging Ruby code. zed-custom's name for the adapter (in the UI and `debug.json`) is `rdbg`, and under the hood, it uses the [`debug`](https://github.com/ruby/debug) gem. The extension uses the [same activation logic](#language-server-activation) as the language servers.
