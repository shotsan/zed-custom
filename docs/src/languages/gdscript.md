...
Godot [GDScript](https://gdscript.com/) language support in zed-custom is provided by the community-maintained [GDScript extension](https://github.com/GDQuest/zed-custom-gdscript).
Report issues to: [https://github.com/GDQuest/zed-custom-gdscript/issues](https://github.com/GDQuest/zed-custom-gdscript/issues)
...
1. Inside your Godot editor, open Editor Settings, look for `Text Editor -> External` and set the following options:
   - Exec Path: `/path/to/zed-custom`
   - Exec Flags: `{project} {file}:{line}:{col}`
   - Use External Editor: "✅ On"
2. Open any \*.gd file through Godot and zed-custom will launch.
