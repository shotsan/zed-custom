...
By default clang and gcc will recognize `*.C` and `*.H` (uppercase extensions) as C++ and not C and so zed-custom too follows this convention. If you are working with a C-only project (perhaps one with legacy uppercase pathing like `FILENAME.C`) you can override this behavior by adding this to your settings:
...
By default zed-custom will use the `clangd` language server for formatting C code like the `clang-format` CLI tool. To configure this you can add a `.clang-format` file. For example:
