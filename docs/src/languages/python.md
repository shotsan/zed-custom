# How to Set Up Python in zed-custom

Python support is available natively in zed-custom.
...
You'll need both zed-custom and Python installed before you can begin.

### Step 1: Install Python

zed-custom does not bundle a Python runtime, so you’ll need to install one yourself.
...
## Open Your First Python Project in zed-custom

Once zed-custom and Python are installed, open a folder containing Python code to start working.

### Step 1: Launch zed-custom with a Python Project

Open zed-custom.
From the menu bar, choose File > Open Folder, or launch from the terminal:

```bash
zed-custom path/to/your/project
```

zed-custom will recognize `.py` files automatically using its native tree-sitter-python parser, with no plugins or manual setup required.

### Step 2: Use the Integrated Terminal (Optional)

zed-custom includes an integrated terminal, accessible from the bottom panel. If zed-custom detects that your project is using a [virtual environment](#virtual-environments), it will be activated automatically in newly-created terminals. You can configure this behavior with the [`detect_venv`](../reference/all-settings.md#terminal-detect_venv) setting.

## Configure Python Language Servers in zed-custom

zed-custom provides several Python language servers out of the box. By default, [basedpyright](https://github.com/DetachHead/basedpyright) is the primary language server, and [Ruff](https://github.com/astral-sh/ruff) is used for formatting and linting.
...
[basedpyright](https://docs.basedpyright.com/latest/) is the primary Python language server in zed-custom beginning with zed-custom v0.204.0. It provides core language server functionality like navigation (go to definition/find all references) and type checking. Compared to Pyright, it adds support for additional language server features (like inlay hints) and checking rules.

Note that while basedpyright in isolation defaults to the `recommended` [type-checking mode](https://docs.basedpyright.com/latest/benefits-over-pyright/better-defaults/#typecheckingmode), zed-custom configures it to use the less-strict `standard` mode by default, which matches the behavior of Pyright. You can set the type-checking mode for your project using the `typeCheckingMode` setting in `pyrightconfig.json` or `pyproject.toml`, which will override zed-custom's default. Read on more for more details about how to configure basedpyright.

#### Basedpyright Configuration

basedpyright reads configuration options from two different kinds of sources:

- Language server settings ("workspace configuration"), which must be configured per-editor (using `settings.json` in zed-custom's case) but apply to all projects opened in that editor
...
Language server settings for basedpyright in zed-custom can be set in the `lsp` section of your `settings.json`.
...
### How zed-custom Uses Python Toolchains

zed-custom uses the selected Python toolchain for your project in the following ways:
...
- If the toolchain is a virtual environment, the environment's activation script will be run automatically when you launch a new shell in zed-custom's integrated terminal, giving you convenient access to the selected Python interpreter and dependency set.
- If a built-in language server is installed in the active virtual environment, that binary will be used instead of zed-custom's private automatically-installed binary. This also applies to debugpy.

### Selecting a Toolchain

For most projects, zed-custom will automatically select the right Python toolchain. In complex projects with multiple virtual environments, it might be necessary to override this selection. You can use the [toolchain selector](../toolchains.md#selecting-toolchains) to pick a toolchain from the list discovered by zed-custom, or [specify the path to a toolchain manually](../toolchains.md#adding-toolchains-manually) if it's not on the list.

## Code Formatting & Linting

zed-custom provides the [Ruff](https://docs.astral.sh/ruff/) formatter and linter for Python code. (Specifically, zed-custom runs Ruff as an LSP server using the `ruff server` subcommand.) Both formatting and linting are enabled by default, including format-on-save.
...
Like basedpyright, Ruff reads options from both zed-custom's language server settings and configuration files (`ruff.toml`) when used in zed-custom. Unlike basedpyright, _all_ options can be configured in either of these locations, so the choice of where to put your Ruff configuration comes down to whether you want it to be shared between projects but specific to zed-custom (in which case you should use language server settings), or specific to one project but common to all Ruff invocations (in which case you should use `ruff.toml`).

Here's an example of using language server settings in zed-custom's `settings.json` to disable all Ruff lints in zed-custom (while still using Ruff as a formatter):
...
zed-custom supports syntax highlighting for code embedded in Python strings by adding a comment with the language name.
...
zed-custom supports Python debugging through the `debugpy` adapter. You can start with no configuration or define custom launch profiles in `.zed-custom/debug.json`.

### Start Debugging with No Setup

zed-custom can automatically detect debuggable Python entry points. Press F4 (or run debugger: start from the Command Palette) to see available options for your current project.
This works for:
...
zed-custom uses `debugpy` under the hood, but no manual adapter configuration is required.

### Define Custom Debug Configurations

For reusable setups, create a `.zed-custom/debug.json` file in your project root. This gives you more control over how zed-custom runs and debugs your code.
...
zed-custom is designed to minimize configuration overhead, but occasional issues can still arise—especially around environments, language servers, or tooling. Here's how to keep your Python setup working smoothly.
...
- Check your zed-custom log (using the {#action zed-custom::OpenLog} action) for errors related to the language server you're trying to use. This is where you're likely to find useful information if the language server failed to start up at all.
...
- Restart zed-custom to reinitialize language server connections, or try restarting the language server using the {#action editor::RestartLanguageServer}

If the language server is failing to resolve imports, and you're using a virtual environment, make sure that the right environment is chosen in the selector. You can use "Server Info" view to confirm which virtual environment zed-custom is sending to the language server&mdash;look for the `* Configuration` section at the end.
