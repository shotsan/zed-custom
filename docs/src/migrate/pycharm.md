# How to Migrate from PyCharm to zed-custom

This guide covers how to set up zed-custom if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.
...
zed-custom also supports per-project settings. Create a `.zed-custom/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in zed-custom. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how zed-custom compares to PyCharm.
...
### Different Keybindings (PyCharm → zed-custom)

| Action                 | PyCharm     | zed-custom (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
zed-custom doesn't index. You open a folder and start working immediately. File search and navigation work instantly regardless of project size. For many PyCharm users, this alone is reason enough to switch—no more waiting, no more "Indexing paused" interruptions.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. zed-custom delegates this work to language servers, which may not analyze as deeply or as broadly.
...
zed-custom uses the Language Server Protocol (LSP) for code intelligence. For Python, zed-custom provides several language servers out of the box:
...
zed-custom handles virtual environments through its toolchain system:

- zed-custom automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in zed-custom—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching zed-custom

> **Tip:** If basedpyright shows import errors for packages you've installed, check that zed-custom has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
zed-custom has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. zed-custom discovers environments but doesn't create them.
...
zed-custom has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from zed-custom's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to zed-custom keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). zed-custom uses a similar concept called "docks":

| PyCharm Tool Window | zed-custom Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | -------------- | --------------------------- |
...
Both PyCharm and zed-custom offer integrated debugging, but the experience differs:

- zed-custom uses `debugpy` (the same debug adapter that VS Code uses)
...
zed-custom can automatically detect debuggable entry points. Press `F4` to see available options, including:

- Python scripts
- Modules
- pytest tests

For more control, create a `.zed-custom/debug.json` file:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. zed-custom provides test running through:
...
Several features that require plugins in PyCharm are built into zed-custom:
...
### What's Not in zed-custom

To set expectations clearly, here's what PyCharm offers that zed-custom doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside zed-custom for your Python editing
...
- **Remote interpreters** — zed-custom has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Collaboration in zed-custom vs. PyCharm

PyCharm offers Code With Me as a separate plugin for collaboration. zed-custom has collaboration built into the core experience.
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), zed-custom offers similar capabilities with more flexibility.
...
- [Python in zed-custom](../languages/python.md) — Python-specific setup and configuration
