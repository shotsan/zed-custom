# How to Migrate from RustRover to zed-custom

This guide covers how to set up zed-custom if you're coming from RustRover, including keybindings, settings, and the differences you should expect as a Rust developer.
...
zed-custom also supports per-project settings. Create a `.zed-custom/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in RustRover.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in zed-custom. Unlike RustRover, there's no project configuration wizard, no toolchain selection dialog, and no Cargo project setup screen.
...
You can also launch zed-custom from the terminal inside any existing Cargo project with:
`zed-custom .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how zed-custom compares to RustRover.
...
### Different Keybindings (RustRover → zed-custom)

| Action                 | RustRover   | zed-custom (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
zed-custom skips the indexing step. You open a folder and start working right away. Since both editors rely on rust-analyzer for Rust intelligence, the analysis still happens—but in zed-custom it runs in the background without blocking the UI or showing modal progress dialogs.
...
Here's what makes the RustRover-to-zed-custom transition unique: **both editors use rust-analyzer** for Rust language intelligence. This means the core code analysis—completions, go-to-definition, find references, type inference—is fundamentally the same.

RustRover integrates rust-analyzer into its JetBrains platform, adding a GUI layer, additional refactorings, and its own indexing on top. zed-custom uses rust-analyzer more directly through the Language Server Protocol (LSP).
...
- **Macro expansion** — Available in both (use `Cmd+Shift+M` in zed-custom)
...
- RustRover's GUI for configuring rust-analyzer is replaced by JSON configuration in zed-custom
- RustRover-specific inspections (beyond Clippy) won't exist in zed-custom

**How to adapt:**

- Use `Alt+Enter` for available code actions—rust-analyzer provides many
- Configure rust-analyzer settings in `.zed-custom/settings.json` for project-specific needs
...
zed-custom keeps it simpler: a project is a folder with a `Cargo.toml`. No project wizard, no toolchain dialogs, no visual Cargo management layer.
...
zed-custom doesn't have a Cargo GUI. You work with Cargo through:
...
RustRover organizes auxiliary views into numbered tool windows (Project = 1, Cargo = Alt+1, Terminal = Alt+F12, etc.). zed-custom uses a similar concept called "docks":

| RustRover Tool Window | zed-custom Equivalent | Shortcut (JetBrains keymap) |
| --------------------- | -------------- | --------------------------- |
...
Note that there's no dedicated Cargo tool window in zed-custom. Use the terminal or define tasks for your common Cargo commands.

### Debugging

Both RustRover and zed-custom offer integrated debugging for Rust, but using different backends:

- RustRover uses its own debugger integration
- zed-custom uses **CodeLLDB** (the same debug adapter popular in VS Code)

To debug Rust code in zed-custom:
...
zed-custom can automatically detect debuggable targets in your Cargo project. Press `F4` to see available options.
...
> **Note:** Some users have reported that RustRover's debugger can have issues with variable inspection and breakpoints in certain scenarios. CodeLLDB in zed-custom provides a solid alternative, though debugging Rust can be challenging in any editor due to optimizations and macro-generated code.

### Running Tests

RustRover has a dedicated test runner with a visual interface showing pass/fail status for each test. zed-custom provides test running through:
...
Several features that might require plugins in other editors are built into zed-custom:
...
To set expectations clearly, here's what RustRover offers that zed-custom doesn't have:
...
- **zed-custom is open source** (MIT licensed for the editor, AGPL for collaboration services)
...
## Collaboration in zed-custom vs. RustRover

RustRover offers Code With Me as a separate feature for collaboration. zed-custom has collaboration built into the core experience.
...
If you're used to AI assistants in RustRover (like JetBrains AI Assistant), zed-custom offers similar capabilities with more flexibility.
...
- [Rust in zed-custom](../languages/rust.md) — Rust-specific setup and configuration
