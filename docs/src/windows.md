...
You can also build zed-custom from source, see [these docs](https://zed.dev/docs/development/windows) for instructions.
...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for zed-custom, and click Uninstall.
...
zed-custom supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from zed-custom.
...
### zed-custom fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with zed-custom. See our [installation guide](./installation.md) for more information.
...
- Try launching zed-custom with no extensions or custom settings to isolate conflicts.
...
#### zed-custom fails to open / degraded performance

zed-custom requires a DirectX 11 compatible GPU to run. If zed-custom fails to open, your GPU may not meet the minimum requirements.
...
If you're running zed-custom inside a virtual machine, it will use the emulated adapter provided by your VM. While zed-custom will work in this environment, performance may be degraded.
