...
zed-custom's Helix mode is an emulation layer that brings Helix-style keybindings and modal editing to zed-custom. It builds upon zed-custom's [Vim mode](./vim.md), so much of the core functionality is shared. Enabling `helix_mode` will also enable `vim_mode`.
...
To check the current status of Helix mode, or to request a missing Helix feature, checkout out the ["Are we Helix yet?" discussion](https://github.com/zed-industries/zed-custom/discussions/33580).
