...
> **DISCLAIMER**: This is not documentation for [configuring zed-custom's debugger](../debugger.md).
> Rather, it is intended to provide information on how to use a debugger while developing zed-custom itself to both zed-custom employees and external contributors.

## Using zed-custom's built-in debugger

While the zed-custom project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug zed-custom with, one for GDB and one for LLDB. Select the configuration you want and zed-custom will build and launch the binary.
...
Therefore, in order to use a debugger to it's fullest extent when debugging a release build, you must compile a new zed-custom binary, with full debug info.
...
> If you wish to avoid passing the `--config` flag on every invocation of `cargo`. You may also change the section in the [root `Cargo.toml`](https://github.com/zed-industries/zed-custom/blob/main/Cargo.toml)
>
> from
>
> ```toml
...
## Running zed-custom with a shell debugger GDB/LLDB

### Background

When installing rust through rustup, (the recommended way to do so when developing zed-custom, see the documentation for getting started on your platform [here](../development.md))
...
### Usage with zed-custom

After following the steps above for including full debug info when compiling zed-custom,
You can either run `rust-gdb` or `rust-lldb` on the compiled zed-custom binary after building it with `cargo build`, by running one of the following commands:

```
rust-gdb target/debug/zed-custom
rust-lldb target/debug/zed-custom
```

Alternatively, you can attach to a running instance of zed-custom (such as an instance of zed-custom started using `cargo run`) by running one of the following commands:
...
Where `<pid>` is the process ID of the zed-custom instance you want to attach to.

To get the process ID of a running zed-custom instance, you can use your systems process management tools such as `Task Manager` on windows or `Activity Monitor` on macOS.

Alternatively, you can run the `ps aux | grep zed-custom` command on macOS and Linux or `Get-Process | Select-Object Id, ProcessName` in an instance of PowerShell on Windows.

#### Debugging Panics and Crashes

Debuggers can be an excellent tool for debugging the cause of panics and crashes in all programs, including zed-custom.
...
You can find additional information on debugging zed-custom crashes [here](./debugging-crashes.md).
