...
zed-custom comes with a built-in icon theme, with more icon themes available as extensions.
...
More icon themes are available from the Extensions page, which you can access via the command palette with `zed-custom: extensions` or the [zed-custom website](https://zed.dev/extensions?filter=icon-themes).
...
Just like with themes, zed-custom allows for configuring different icon themes for light and dark mode.
...
See: [Developing zed-custom Icon Themes](./extensions/icon-themes.md)
