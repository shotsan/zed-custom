...
zed-custom supports ways to spawn (and rerun) commands using its integrated [terminal](./terminal.md) to output the results. These commands can read a limited subset of zed-custom state (such as a path to the file currently being edited or selected text).
...
- in the global `tasks.json` file; such tasks are available in all zed-custom projects you work on. This file is usually located in `~/.config/zed-custom/tasks.json`. You can edit them by using the `zed-custom: open tasks` action.
- in the worktree-specific (local) `.zed-custom/tasks.json` file; such tasks are available only when working on a project with that worktree included. You can edit worktree-specific tasks by using the `zed-custom: open project tasks` action.
...
zed-custom tasks act just like your shell; that also means that you can reference environmental variables via sh-esque `$VAR_NAME` syntax. A couple of additional environmental variables are set for your convenience.
...
zed-custom supports overriding the default action for inline runnable indicators via workspace-local and global `tasks.json` file with the following precedence hierarchy:
