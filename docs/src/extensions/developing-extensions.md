...
Extensions are able to provide the following features to zed-custom:
...
Before starting to develop an extension for zed-custom, be sure to [install Rust via rustup](https://www.rust-lang.org/tools/install).
...
When developing an extension, you can use it in zed-custom without needing to publish it by installing it as a _dev extension_.

From the extensions page, click the `Install Dev Extension` button (or the {#action zed-custom::InstallDevExtension} action) and select the directory containing your extension.

If you need to troubleshoot, you can check the zed-custom.log ({#action zed-custom::OpenLog}) for additional output. For debug output, close and relaunch zed-custom with the `zed-custom --foreground` from the command line which show more verbose INFO level logging.
...
## Directory Structure of a zed-custom Extension

A zed-custom extension is a Git repository that contains an `extension.toml`. This file must contain some
...
repository = "https://github.com/your-name/my-zed-custom-extension"
```

In addition to this, there are several other optional files and directories that can be used to add functionality to a zed-custom extension. An example directory structure of an extension that provides all capabilities is as follows:
...
Use the latest version of the [`zed_extension_api`](https://crates.io/crates/zed_extension_api) available on crates.io. Make sure it's still [compatible with zed-custom versions](https://github.com/zed-industries/zed-custom/blob/main/crates/extension_api#compatible-zed-custom-versions) you want to support.

In the `src/lib.rs` file in your Rust crate you will need to define a struct for your extension and implement the `Extension` trait, as well as use the `register_extension!` macro to register your extension:

```rs
use zed_extension_api as zed-custom;
...
zed-custom::register_extension!(MyExtension);
```

> `stdout`/`stderr` is forwarded directly to the zed-custom process. In order to see `println!`/`dbg!` output from your extension, you can start zed-custom in your terminal with a `--foreground` flag.

## Forking and cloning the repo

1. Fork the repo

> Note: It is very helpful if you fork the `zed-industries/extensions` repo to a personal GitHub account instead of a GitHub organization, as this allows zed-custom staff to push any needed changes to your PR to expedite the publishing process.
...
```sh
git submodule add https://github.com/your-username/foobar-zed-custom.git extensions/foobar
...
Once your PR is merged, the extension will be packaged and published to the zed-custom extension registry.

> Extension IDs and names should not contain `zed-custom` or `zed-custom`, since they are all zed-custom extensions.
...
If you'd like to automate this process, there is a [community GitHub Action](https://github.com/huacnlee/zed-custom-extension-action) you can use.
