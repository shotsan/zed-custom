...
zed-custom includes a Vim emulation layer known as "vim mode". On this page, you will learn how to turn zed-custom's vim mode on or off, what tools and commands zed-custom provides to help you navigate and edit your code, and generally how to make the most of vim mode in zed-custom.

You'll learn how to:

- Understand the core differences between zed-custom's vim mode and traditional Vim
- Enable or disable vim mode
- Make the most of zed-custom-specific features within vim mode
- Customize vim mode key bindings
- Configure vim mode settings

Whether you're new to vim mode or an experienced Vim user looking to optimize your zed-custom experience, this guide will help you harness the full power of modal editing in zed-custom.

## zed-custom's vim mode design

Vim mode tries to offer a familiar experience to Vim users: it replicates the behavior of motions and commands precisely when it makes sense and uses zed-custom-specific functionality to provide an editing experience that "just works" without requiring configuration on your part.
...
So, zed-custom's vim mode does not replicate Vim one-to-one, but it meshes Vim's modal design with zed-custom's modern features to provide a more fluid experience. It's also configurable, so you can add your own key bindings or override the defaults.

### Core differences

There are four types of features in vim mode that use zed-custom's core functionality, leading to some differences in behavior:

1. **Motions**: vim mode uses zed-custom's semantic parsing to tune the behavior of motions per language. For example, in Rust, jumping to matching bracket with `%` works with the pipe character `|`. In JavaScript, `w` considers `$` to be a word character.
2. **Visual block selections**: vim mode uses zed-custom's multiple cursor to emulate visual block selections, making block selections a lot more flexible. For example, anything you insert after a block selection updates on every line in real-time, and you can add or remove cursors anytime.
3. **Macros**: vim mode uses zed-custom's recording system for vim macros. So, you can capture and replay more complex actions, like autocompletion.
4. **Search and replace**: vim mode uses zed-custom's search system, so, the syntax for regular expressions is slightly different compared to Vim. [Head to the Regex differences section](#regex-differences) for details.

> **Note:** The foundations of zed-custom's vim mode should already cover many use cases, and we're always looking to improve it. If you find missing features that you rely on in your workflow, please [file an issue on GitHub](https://github.com/zed-industries/zed-custom/issues).

## Enabling and disabling vim mode

When you first open zed-custom, you'll see a checkbox on the welcome screen that allows you to enable vim mode.
...
## zed-custom-specific features

zed-custom is built on a modern foundation that (among other things) uses Tree-sitter and language servers to understand the content of the file you're editing and supports multiple cursors out of the box.

Vim mode has several "core zed-custom" key bindings that will help you make the most of zed-custom's specific feature set.
...
Tree-sitter is a powerful tool that zed-custom uses to understand the structure of your code. zed-custom provides motions that change the current cursor position, and text objects that can be used as the target of actions.
...
These commands help you manage multiple cursors in zed-custom.
...
The following commands help you bring up zed-custom's completion menu, request a suggestion from GitHub Copilot, or open the inline AI assistant without leaving insert mode.
...
zed-custom's vim mode includes some features that are usually provided by very popular plugins in the Vim ecosystem:
...
zed-custom offers two different strategies for selecting text surrounded by any quote, or any bracket. These text objects are **not enabled by default** and must be configured in your keymap to be used.
...
Vim mode allows you to open zed-custom's command palette with `:`. You can then type to access any usual zed-custom command. Additionally, vim mode adds aliases for popular Vim commands to ensure your muscle memory transfers to zed-custom. For example, you can write `:w` or `:write` to save the file.
...
> **Note**: We don't emulate the full power of Vim's command line yet. In particular, commands currently do not support arguments. Please [file issues on GitHub](https://github.com/zed-industries/zed-custom) as you find things that are missing from the command palette.
...
| `:cq`          | Quit completely (close all running instances of zed-custom) |
...
These ex commands open zed-custom's various panels and windows.
...
This command replaces text. It emulates the substitute command in vim. The substitute command uses regular expressions, and zed-custom uses a slightly different syntax than vim. You can learn more about zed-custom's syntax below, [in the regex differences section](#regex-differences). zed-custom will replace only the first occurrence of the search pattern in the current line. To replace all matches append the `g` flag.
...
As any zed-custom command is available, you may find that it's helpful to remember mnemonics that run the correct command. For example:

- `:diffs` for "toggle all hunk diffs"
- `:cpp` for "copy path to file"
- `:crp` for "copy relative path"
- `:reveal` for "reveal in finder"
- `:zlog` for "open zed-custom log"
- `:clank` for "cancel language server work"

## Customizing key bindings

In this section, we'll learn how to customize the key bindings of zed-custom's vim mode. You'll learn:
...
zed-custom's key bindings are evaluated only when the `"context"` property matches your location in the editor. For example, if you add key bindings to the `"Editor"` context, they will only work when you're editing a file. If you add key bindings to the `"Workspace"` context, they will work everywhere in zed-custom. Here's an example of a key binding that saves when you're editing a file:
...
```json [keymap]
// This key binding will work when you're editing a file. It comes built into zed-custom by default as the workspace: save command.
{
  "context": "Workspace",
  "bindings": {
...
In non-modal text editors, cursor navigation typically wraps when moving past line ends. zed-custom, however, handles this behavior exactly like Vim by default: the cursor stops at line boundaries. If you prefer your cursor to wrap between lines, override these keybindings:
...
### Restoring common text editing and zed-custom keybindings
...
## Useful core zed-custom settings for vim mode

Here are a few general zed-custom settings that can help you fine-tune your Vim experience:
...
zed-custom uses a different regular expression engine from Vim. This means that you will have to use a different syntax in some cases. Here are the most common differences:

- **Capture groups**: Vim uses `\(` and `\)` to represent capture groups, in zed-custom these are `(` and `)`. On the flip side, in Vim, `(` and `)` represent literal parentheses, but in zed-custom these must be escaped to `\(` and `\)`.
- **Matches**: When replacing, Vim uses the backslash character followed by a number to represent a matched capture group. For example, `\1`. zed-custom uses the dollar sign instead. So, when in Vim you use `\0` to represent the entire match, in zed-custom the syntax is `$0` instead. Same for numbered capture groups: `\1` in Vim is `$1` in zed-custom.
- **Global option**: By default, in Vim, regex searches only match the first occurrence on a line, and you append `/g` at the end of your query to find all matches. In zed-custom, regex searches are global by default.
- **Case sensitivity**: Vim uses `/i` to indicate a case-insensitive search. In zed-custom you can either write `(?i)` at the start of the pattern or toggle case-sensitivity with the shortcut {#kb search::ToggleCaseSensitive}.

> **Note**: To help with the transition, the command palette will fix parentheses and replace groups for you when you write a Vim-style substitute command, `:%s//`. So, zed-custom will convert `%s:/\(a\)(b)/\1/` into a search for "(a)\(b\)" and a replacement of "$1".

For the full syntax supported by zed-custom's regex engine [see the regex crate documentation](https://docs.rs/regex/latest/regex/#syntax).
