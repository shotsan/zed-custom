...
zed-custom supports many external agents, including CLI-based ones, through the [Agent Client Protocol (ACP)](https://agentclientprotocol.com).
...
> Note that zed-custom's interaction with external agents is strictly UI-based; the billing, legal, and terms arrangement is directly between you and the agent provider.
> zed-custom does not charge for use of external agents, and our [zero-data retention agreements/privacy guarantees](./ai-improvement.md) are **_only_** applicable for zed-custom's hosted models.

## Gemini CLI {#gemini-cli}

zed-custom provides the ability to run [Gemini CLI](https://github.com/google-gemini/gemini-cli) directly in the [agent panel](./agent-panel.md).
...
If you'd like to bind this to a keyboard shortcut, you can do so by editing your `keymap.json` file via the `zed-custom: open keymap file` command to include:
...
The first time you create a Gemini CLI thread, zed-custom will install [@google/gemini-cli](https://github.com/google-gemini/gemini-cli).
This installation is only available to zed-custom and is kept up to date as you use the agent.

By default, zed-custom will use this managed version of Gemini CLI even if you have it installed globally.
...
zed-custom does not see your OAuth or access tokens in this case.

You can also use the "Gemini API Key".
If you select this, and have the `GEMINI_API_KEY` set, then we will use that.
Otherwise zed-custom will prompt you for an API key which will be stored securely in your keychain, and used to start Gemini CLI from within zed-custom.
...
Similar to zed-custom's first-party agent, you can use Gemini CLI to do anything that you need.
...
Similar to Gemini CLI, you can also run [Claude Code](https://www.anthropic.com/claude-code) directly via zed-custom's [agent panel](./agent-panel.md).
Under the hood, zed-custom runs Claude Code and communicate to it over ACP, through [a dedicated adapter](https://github.com/zed-industries/claude-code-acp).
...
As of version `0.202.7`, authentication to zed-custom's Claude Code installation is decoupled entirely from zed-custom's agent.
That is to say, an Anthropic API key added via the [zed-custom Agent's settings](./llm-providers.md#anthropic) will _not_ be utilized by Claude Code for authentication and billing.
...
The first time you create a Claude Code thread, zed-custom will install [@zed-industries/claude-code-acp](https://github.com/zed-industries/claude-code-acp).
This installation is only available to zed-custom and is kept up to date as you use the agent.

zed-custom will always use this managed version of the Claude Code adapter, which includes a vendored version of the Claude Code CLI, even if you have it installed globally.
...
Similar to zed-custom's first-party agent, you can use Claude Code to do anything that you need.
And to give it context, you can @-mention files, recent threads, diagnostics, symbols, and more.

In complement to talking to it [over ACP](https://agentclientprotocol.com), zed-custom relies on the [Claude Code SDK](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-overview) to support some of its specific features.
...
Claude Code in zed-custom will automatically use any `CLAUDE.md` file found in your project root, project subdirectories, or root `.claude` directory.

If you don't have a `CLAUDE.md` file, you can ask Claude Code to create one for you through the `init` slash command.

## Codex CLI

You can also run [Codex CLI](https://github.com/openai/codex) directly via zed-custom's [agent panel](./agent-panel.md).
Under the hood, zed-custom runs Codex CLI and communicates to it over ACP, through [a dedicated adapter](https://github.com/zed-industries/codex-acp).

### Getting Started

As of version `0.208`, you should be able to use Codex directly from zed-custom.
...
Authentication to zed-custom's Codex installation is decoupled entirely from zed-custom's agent.
That is to say, an OpenAI API key added via the [zed-custom Agent's settings](./llm-providers.md#openai) will _not_ be utilized by Codex for authentication and billing.
...
The first time you create a Codex thread, zed-custom will install [codex-acp](https://github.com/zed-industries/codex-acp).
This installation is only available to zed-custom and is kept up to date as you use the agent.

zed-custom will always use this managed version of Codex even if you have it installed globally.

### Usage

Similar to zed-custom's first-party agent, you can use Codex to do anything that you need.
...
Starting from `v0.221.x`, [the ACP Registry](https://agentclientprotocol.com/registry) is the preferred way to install external agents in zed-custom.
...
Add more external agents to zed-custom by installing [Agent Server extensions](../extensions/agent-servers.md).

See what agents are available by filtering for "Agent Servers" in the extensions page, which you can access via the command palette with `zed-custom: extensions`, or the [zed-custom website](https://zed.dev/extensions?filter=agent-servers).
...
#### Using it in zed-custom

Use the `zed-custom: acp registry` command to quickly go to the ACP Registry page.
...
When using external agents in zed-custom, you can access the debug view via with `dev: open acp logs` from the Command Palette.
This lets you see the messages being sent and received between zed-custom and the agent.
...
Note that for external agents, access to MCP servers [installed from zed-custom](./mcp.md) may vary depending on the ACP implementation.
